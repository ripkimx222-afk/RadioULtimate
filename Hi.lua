...
local KNOWN_SERVICES = {
  ...
  ServerScriptService=true, ServerStorage=true, StarterGui=true, StarterPack=true,
  StarterPlayer=true, Teams=true, SoundService=true, Chat=true, TextChatService=true,
  MarketplaceService=true, HttpService=true, TweenService=true, UserInputService=true,
  RunService=true, Debris=true, PathfindingService=true, CollectionService=true,
}

local function buildLuaPath(inst)
  local parts = {}
  local cur = inst
  ...
  local first = parts[1]
  local str
  if KNOWN_SERVICES[first.Name] then
    str = 'game:GetService("' .. first.Name .. '")'
  else
    str = "game." .. first.Name
  end
  for i = 2, #parts do
    local nm = parts[i].Name
    if nm:match("^[%a_][%w_]*$") then
      str = str .. "." .. nm
    else
      str = str .. '["' .. nm .. '"]'
    end
...
local CONTAINER_CLASSES = {
  Folder=true, Script=true, LocalScript=true, ModuleScript=true, Model=true,
  Configuration=true, Tool=true,
}

local function openExplorerWindow()
  local old = PG:FindFirstChild("RiseExplorerWindow")
  if old then old:Destroy() end

  local THEME = TH[CFG.theme] or TH[1]
  local BG,PAN,PANA,AC,ACS,TX,TXM,BD,ST = THEME[2],THEME[3],THEME[4],THEME[5],THEME[6],THEME[7],THEME[8],THEME[9],THEME[10]
  ...
  mainStroke.Color = AC; mainStroke.Thickness = 1

  local hdr = Instance.new("Frame"); hdr.Size = UDim2.new(1,0,0,40)
  hdr.BackgroundTransparency = 1; hdr.ZIndex = 2; hdr.Parent = main
  local ttl = Instance.new("TextLabel"); ttl.Text = "rise.explorer"
  ttl.Size = UDim2.new(0,200,1,0); ttl.Position = UDim2.new(0,12,0,0)
  ttl.BackgroundTransparency = 1; ttl.Font = FONT_BOLD; ttl.TextSize = SZ(16)
  ttl.TextColor3 = TX; ttl.TextXAlignment = Enum.TextXAlignment.Left; ttl.ZIndex = 3; ttl.Parent = hdr
  local closeBtn = Instance.new("TextButton"); closeBtn.Text = "X"
  closeBtn.Size = UDim2.new(0,30,0,26); closeBtn.AnchorPoint = Vector2.new(1,0)
  closeBtn.Position = UDim2.new(1,-8,0,7)
  closeBtn.BackgroundColor3 = ST; closeBtn.TextColor3 = Color3.new(1,1,1)
  closeBtn.Font = FONT_BOLD; closeBtn.TextSize = SZ(12)
  closeBtn.BorderSizePixel = 0; closeBtn.ZIndex = 3; closeBtn.Parent = hdr
  ...
  local searchBar = Instance.new("Frame"); searchBar.Size = UDim2.new(1,-24,0,28)
  searchBar.Position = UDim2.new(0,12,0,44)
  searchBar.BackgroundColor3 = PAN; searchBar.BorderSizePixel = 0; searchBar.ZIndex = 2; searchBar.Parent = main
  ...
  searchBox.ZIndex = 3; searchBox.Parent = searchBar
  local searchClear = Instance.new("TextButton"); searchClear.Text = "X"
  searchClear.Size = UDim2.new(0,24,0,20); searchClear.Position = UDim2.new(1,-30,0,4)
  searchClear.BackgroundColor3 = rgb(60,60,60); searchClear.TextColor3 = Color3.new(1,1,1)
  searchClear.Font = FONT_BOLD; searchClear.TextSize = SZ(10)
  searchClear.BorderSizePixel = 0; searchClear.ZIndex = 3; searchClear.Parent = searchBar
  Instance.new("UICorner", searchClear).CornerRadius = UDim.new(0,4)

  local pathBar = Instance.new("Frame"); pathBar.Size = UDim2.new(1,-24,0,22)
  pathBar.Position = UDim2.new(0,12,0,78)
  pathBar.BackgroundTransparency = 1; pathBar.ZIndex = 2; pathBar.Parent = main
  local backBtn = Instance.new("TextButton")
  backBtn.Size = UDim2.new(0,50,1,0); backBtn.Text = "< Up"
  backBtn.TextSize = SZ(11); backBtn.Font = FONT_BOLD
  backBtn.BackgroundColor3 = PANA; backBtn.TextColor3 = AC
  backBtn.BorderSizePixel = 0; backBtn.ZIndex = 3; backBtn.Parent = pathBar
  Instance.new("UICorner", backBtn).CornerRadius = UDim.new(0,6)
  local pathLabel = Instance.new("TextLabel")
  pathLabel.BackgroundTransparency = 1
  pathLabel.Position = UDim2.new(0,58,0,0); pathLabel.Size = UDim2.new(1,-58,1,0)
  pathLabel.Text = "game.Workspace"; pathLabel.TextSize = SZ(10)
  pathLabel.Font = FONT_REG; pathLabel.TextColor3 = TXM
  pathLabel.TextXAlignment = Enum.TextXAlignment.Left
  pathLabel.TextTruncate = Enum.TextTruncate.AtEnd
  pathLabel.ZIndex = 3; pathLabel.Parent = pathBar

  local holdBar = Instance.new("Frame"); holdBar.Size = UDim2.new(1,-24,0,30)
  holdBar.Position = UDim2.new(0,12,0,104)
  holdBar.BackgroundColor3 = rgb(80,60,140); holdBar.BorderSizePixel = 0
  holdBar.Visible = false; holdBar.ZIndex = 2; holdBar.Parent = main
  Instance.new("UICorner", holdBar).CornerRadius = UDim.new(0,6)
  local holdLbl = Instance.new("TextLabel"); holdLbl.Text = ""
  holdLbl.Size = UDim2.new(1,-140,1,0); holdLbl.Position = UDim2.new(0,8,0,0)
  holdLbl.BackgroundTransparency = 1; holdLbl.TextColor3 = Color3.new(1,1,1)
  holdLbl.Font = FONT_BOLD; holdLbl.TextSize = SZ(10)
  holdLbl.TextXAlignment = Enum.TextXAlignment.Left; holdLbl.TextTruncate = Enum.TextTruncate.AtEnd
  holdLbl.ZIndex = 3; holdLbl.Parent = holdBar
  local dropBtn = Instance.new("TextButton"); dropBtn.Text = "Drop Here"
  dropBtn.Size = UDim2.new(0,80,0,22); dropBtn.Position = UDim2.new(1,-166,0.5,-11)
  dropBtn.BackgroundColor3 = rgb(40,160,70); dropBtn.TextColor3 = Color3.new(1,1,1)
  dropBtn.Font = FONT_BOLD; dropBtn.TextSize = SZ(9)
  dropBtn.BorderSizePixel = 0; dropBtn.ZIndex = 3; dropBtn.Parent = holdBar
  Instance.new("UICorner", dropBtn).CornerRadius = UDim.new(0,5)
  local cancelHoldBtn = Instance.new("TextButton"); cancelHoldBtn.Text = "Cancel"
  cancelHoldBtn.Size = UDim2.new(0,70,0,22); cancelHoldBtn.Position = UDim2.new(1,-78,0.5,-11)
  cancelHoldBtn.BackgroundColor3 = rgb(150,40,40); cancelHoldBtn.TextColor3 = Color3.new(1,1,1)
  cancelHoldBtn.Font = FONT_BOLD; cancelHoldBtn.TextSize = SZ(9)
  cancelHoldBtn.BorderSizePixel = 0; cancelHoldBtn.ZIndex = 3; cancelHoldBtn.Parent = holdBar
  Instance.new("UICorner", cancelHoldBtn).CornerRadius = UDim.new(0,5)

  local scroll = Instance.new("ScrollingFrame")
  scroll.Size = UDim2.new(1,-24,1,-146); scroll.Position = UDim2.new(0,12,0,104)
  scroll.BackgroundTransparency = 1; scroll.BorderSizePixel = 0; scroll.ScrollBarThickness = 4
  scroll.ScrollBarImageColor3 = AC; scroll.ZIndex = 2; scroll.Parent = main
  ...
  propModal.Size = UDim2.new(0,280,0,360)
  ...
  propScroll.ZIndex = 51; propScroll.Parent = propModal
  ...
  local propCloseBtn = Instance.new("TextButton"); propCloseBtn.Text = "Close"
  propCloseBtn.Size = UDim2.new(1,-16,0,32); propCloseBtn.Position = UDim2.new(0,8,1,-40)
  ...
  propCloseBtn.BorderSizePixel = 0; propCloseBtn.ZIndex = 51; propCloseBtn.Parent = propModal
  ...
  local function updateHoldBar()
    if heldInstance then
      holdBar.Visible = true
      holdLbl.Text = "Holding: " .. heldInstance.Name
      scroll.Position = UDim2.new(0,12,0,140)
      scroll.Size = UDim2.new(1,-24,1,-182)
    else
      holdBar.Visible = false
      scroll.Position = UDim2.new(0,12,0,104)
      scroll.Size = UDim2.new(1,-24,1,-146)
    end
  end

  dropBtn.MouseButton1Click:Connect(function()
    if heldInstance then
      pcall(function() heldInstance.Parent = explorerRoot end)
      heldInstance = nil
      updateHoldBar()
      if renderExplorer then renderExplorer() end
    end
  end)
  cancelHoldBtn.MouseButton1Click:Connect(function()
    heldInstance = nil
    updateHoldBar()
  end)
  ...
  local function openPropPanel(inst)
    ...
    local function field(label, value)
      ...
      row.BackgroundColor3 = PANA; row.BorderSizePixel = 0; row.ZIndex = 52; row.Parent = propScroll
      ...
      l1.TextXAlignment = Enum.TextXAlignment.Left; l1.ZIndex = 53; l1.Parent = row
    ...
    nameRow.BackgroundColor3 = PANA; nameRow.BorderSizePixel = 0; nameRow.ZIndex = 52; nameRow.Parent = propScroll
    ...
    nameBox.ZIndex = 53; nameBox.Parent = nameRow
    ...
    renameBtn.BorderSizePixel = 0; renameBtn.ZIndex = 53; renameBtn.Parent = nameRow
    ...
    local function toggleField(label, getter, setter)
      ...
      track.BorderSizePixel = 0; track.ZIndex = 53; track.Parent = row
      Instance.new("UICorner", track).CornerRadius = UDim.new(1,0)
      local knob = Instance.new("Frame"); knob.Size = UDim2.new(0,16,0,16)
      knob.Position = UDim2.new(0,2,0,2)
      knob.BackgroundColor3 = Color3.new(1,1,1)
      knob.BorderSizePixel = 0; knob.ZIndex = 54; knob.Parent = track
      ...
      b.BackgroundTransparency = 1; b.Size = UDim2.new(1,0,1,0); b.Text = ""; b.ZIndex = 55; b.Parent = row
    ...
    if okPart and isPart then
      ...
      local function numRow(label, getVal, setVal)
        local row = Instance.new("Frame"); row.Size = UDim2.new(1,0,0,32)
        row.BackgroundColor3 = PANA; row.BorderSizePixel = 0; row.ZIndex = 52; row.Parent = propScroll
        Instance.new("UICorner", row).CornerRadius = UDim.new(0,6)
        local tl = Instance.new("TextLabel"); tl.Text = label
        tl.Size = UDim2.new(0,80,1,0); tl.Position = UDim2.new(0,8,0,0)
        tl.BackgroundTransparency = 1; tl.TextColor3 = TX
        tl.Font = FONT_BOLD; tl.TextSize = SZ(11)
        tl.TextXAlignment = Enum.TextXAlignment.Left; tl.ZIndex = 53; tl.Parent = row
        local tBox = Instance.new("TextBox"); tBox.Text = getVal()
        tBox.Size = UDim2.new(0,90,0,24); tBox.Position = UDim2.new(1,-126,0.5,-12)
        tBox.BackgroundColor3 = BG; tBox.TextColor3 = TX
        tBox.Font = FONT_REG; tBox.TextSize = SZ(10)
        tBox.ZIndex = 53; tBox.Parent = row
        ...
        tApply.BorderSizePixel = 0; tApply.ZIndex = 53; tApply.Parent = row
        Instance.new("UICorner", tApply).CornerRadius = UDim.new(0,4)
        tApply.MouseButton1Click:Connect(function() setVal(tBox.Text) end)
      end
      numRow("Transparency", function() return tostring(inst.Transparency) end, function(txt)
        local n = tonumber(txt); if n then pcall(function() inst.Transparency = n end) end
      end)
      numRow("Position", function()
        local p = inst.Position
        return string.format("%.1f,%.1f,%.1f", p.X, p.Y, p.Z)
      end, function(txt)
        local x,y,z = txt:match("([%-%d%.]+)%s*,%s*([%-%d%.]+)%s*,%s*([%-%d%.]+)")
        if x and y and z then
          pcall(function() inst.Position = Vector3.new(tonumber(x), tonumber(y), tonumber(z)) end)
        end
      end)
    end

    local moveBtn = Instance.new("TextButton"); moveBtn.Text = "Move To..."
    moveBtn.Size = UDim2.new(1,0,0,32)
    moveBtn.BackgroundColor3 = rgb(80,60,140); moveBtn.TextColor3 = Color3.new(1,1,1)
    moveBtn.Font = FONT_BOLD; moveBtn.TextSize = SZ(11)
    moveBtn.BorderSizePixel = 0; moveBtn.ZIndex = 52; moveBtn.Parent = propScroll
    ...
    local copyBtn = Instance.new("TextButton"); copyBtn.Text = "Copy Code"
    ...
    copyBtn.BorderSizePixel = 0; copyBtn.ZIndex = 52; copyBtn.Parent = propScroll
    Instance.new("UICorner", copyBtn).CornerRadius = UDim.new(0,6)
    copyBtn.MouseButton1Click:Connect(function()
      local code = buildLuaPath(inst)
      local didCopy = false
      pcall(function() setclipboard(code); didCopy = true end)
      copyBtn.Text = didCopy and "Copied!" or code
      task.wait(1)
    ...
    destroyBtn.BorderSizePixel = 0; destroyBtn.ZIndex = 52; destroyBtn.Parent = propScroll
  ...
  local function makeRowInteractive(btn, inst)
    local lastTap = 0
    local pressStart = 0
    ...
    btn.InputBegan:Connect(function(i)
      if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
        ...
        task.delay(0.55, function()
          if stillDown and tick() - pressStart >= 0.5 then
            openPropPanel(inst)
          end
        end)
      end
  ...
  local function scoreMatch(inst, qlow)
    local nlow = inst.Name:lower()
    if nlow == qlow then return 3 end
    if CONTAINER_CLASSES[inst.ClassName] and nlow:find(qlow, 1, true) then return 2 end
    if nlow:find(qlow, 1, true) then return 1 end
    return 0
  end
  ...
  renderExplorer = function()
    ...
    if query ~= "" then
      ...
      if okd then
        for _, inst in ipairs(desc) do
          local sc = scoreMatch(inst, qlow)
          if sc > 0 then
            table.insert(results, {inst=inst, score=sc})
    ...
    for _, inst in ipairs(children) do
      ...
      expandBtn.ZIndex = 3; expandBtn.Parent = row
      ...
      nameBtn.ZIndex = 3; nameBtn.Parent = row
      makeRowInteractive(nameBtn, inst)
    end
  end
  ...
  searchClear.MouseButton1Click:Connect(function() searchBox.Text = ""; renderExplorer() end)
...
buildUI = function()
  ...
  local THEME = TH[CFG.theme] or TH[1]
  local BG,PAN,PANA,AC,ACS,TX,TXM,BD,ST,DOT,GRAD = THEME[2],THEME[3],THEME[4],THEME[5],THEME[6],THEME[7],THEME[8],THEME[9],THEME[10],THEME[11],THEME[12]
  ...
  main.Position = UDim2.new(0.02,0,-0.95,0)
  ...
  local function showMain()
    ...
    TweenService:Create(main, TweenInfo.new(0.45, Enum.EasingStyle.Quart, Enum.EasingDirection.Out),
      {Position = UDim2.new(0.02,0,0.09,0)}):Play()
  end
  hideMain = function()
  ...
  local function globalTabBtn(label, id)
    ...
    b.MouseButton1Click:Connect(function()
      if activeApp ~= id then
        ...
        task.defer(function()
          if main and main.Parent then main.Visible = true; main.Position = UDim2.new(0.02,0,0.09,0) end
        end)
      end
    end)
    return b
  ...
  if guiMode == "new" then
    side.Size = UDim2.new(1,-16,0,34); side.Position = UDim2.new(0,8,0,76)
  else
    side.Size = UDim2.new(0,88,1,-148); side.Position = UDim2.new(0,8,0,76)
  end
  ...
  if guiMode == "new" then
    sideLay.FillDirection = Enum.FillDirection.Horizontal
    sideLay.VerticalAlignment = Enum.VerticalAlignment.Center
    side.ScrollingDirection = Enum.ScrollingDirection.X
    sideLay:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
      side.CanvasSize = UDim2.new(0,sideLay.AbsoluteContentSize.X+8,0,0)
    end)
  else
    sideLay:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
  ...
  local function renderSidebar()
    ...
    if guiMode == "new" then
      for _,tm in ipairs(TabMeta) do
        ...
        local w = math.max(60, #tm.label*8+24)
        b.Size = UDim2.new(0,w,1,0)
        local active = tm.id == curTab and curView == "songs"
        b.BackgroundColor3 = active and AC or PANA
        b.TextColor3 = active and Color3.new(1,1,1) or TXM
        b.Font = FONT_BOLD; b.TextSize = SZ(11)
        b.BorderSizePixel = 0; b.ZIndex = 52; b.Parent = side
  ...
  if guiMode == "new" then
    content.Size = UDim2.new(1,-16,1,-190); content.Position = UDim2.new(0,8,0,116)
  else
    content.Size = UDim2.new(1,-104,1,-148); content.Position = UDim2.new(0,98,0,76)
  end
    ...
    for _,id in ipairs(ids) do
      if not seen[id] then
        ...
        if s then
          ...
          if match then
            count = count + 1
            local row = Instance.new("Frame"); row.Size = UDim2.new(1,-2,0,58)
            ...
            nm.Size = UDim2.new(1,-16,0,22); nm.Position = UDim2.new(0,8,0,4)
            nm.BackgroundTransparency = 1; nm.TextColor3 = TX
            nm.Font = FONT_REG; nm.TextSize = SZ(11)
            nm.TextXAlignment = Enum.TextXAlignment.Left
            nm.TextTruncate = Enum.TextTruncate.AtEnd
            nm.ZIndex = 54; nm.Parent = row

            local btnRow = Instance.new("Frame")
            btnRow.Size = UDim2.new(1,-16,0,24); btnRow.Position = UDim2.new(0,8,0,28)
            btnRow.BackgroundTransparency = 1; btnRow.ZIndex = 54; btnRow.Parent = row
            ...
            local function cardBtn(txt, w, bg)
              local b = Instance.new("TextButton"); b.Text = txt
              b.Size = UDim2.new(0,w,1,0)
              b.BackgroundColor3 = bg; b.TextColor3 = Color3.new(1,1,1)
              b.Font = FONT_BOLD; b.TextSize = SZ(9)
              b.BorderSizePixel = 0; b.ZIndex = 55; b.Parent = btnRow
              Instance.new("UICorner", b).CornerRadius = UDim.new(0,4)
              return b
            end

            local bExp = cardBtn(btnExp, 34, rgb(180,160,30))
            local bPlay = cardBtn(btnPlay, 34, AC)
            local bPrev = cardBtn(btnPrev, 34, ACS)
            local bEdit = cardBtn(btnEdit, 34, rgb(90,90,90))
            local bCopy = cardBtn(btnCopy, 34, ACS)
            local bRem = cardBtn(btnRem, 34, rgb(80,80,80))
            local bDel = cardBtn(btnDel, 34, ST)
            ...
            bEdit.MouseButton1Click:Connect(function()
              ...
              box.Size = UDim2.new(1,-16,0,22); box.Position = UDim2.new(0,8,0,4)
              box.BackgroundColor3 = BG; box.TextColor3 = TX
              box.Font = FONT_BOLD; box.TextSize = SZ(10)
              box.TextXAlignment = Enum.TextXAlignment.Left
              box.BorderSizePixel = 0; box.ZIndex = 60; box.Parent = row
  ...
  renderSettings = function()
    ...
    action("Launch Explorer", rgb(90,50,150), function(b)
      openExplorerWindow()
    end)
    ...
    pickerRow(T("fontLabel"), fontNames, function() return CFG.font end, function(oi)
      ...
      task.defer(function() if main and main.Parent then main.Visible = true; main.Position = UDim2.new(0.02,0,0.09,0) end end)
    end, 78)
  ...
  if BUILD_OK then
    ...
    warn("[RiseUltimate] buildUI error: " .. tostring(BUILD_ERR))
    pcall(function() gui:Destroy() end)
  end
end

local function showGuiPicker(onChosen)
  local T0 = TH[CFG.theme] or TH[1]
  local pgui = Instance.new("ScreenGui")
  pgui.Name = "RiseGuiPicker"
  pgui.ResetOnSpawn = false
  pgui.DisplayOrder = 999
  pgui.Parent = PG
  ...
  local card = Instance.new("Frame")
  card.Size = UDim2.new(0,280,0,230)
  card.AnchorPoint = Vector2.new(0.5,0.5)
  card.Position = UDim2.new(0.5,0,0.5,0)
  card.BackgroundColor3 = T0[3]
  card.ZIndex = 501
  card.Parent = bg
  Instance.new("UICorner", card).CornerRadius = UDim.new(0,16)
  local cardStroke = Instance.new("UIStroke", card)
  cardStroke.Color = T0[5]; cardStroke.Thickness = 1

  local logoHolder = Instance.new("Frame")
  logoHolder.Size = UDim2.new(0,48,0,48)
  logoHolder.AnchorPoint = Vector2.new(0.5,0)
  logoHolder.Position = UDim2.new(0.5,0,0,16)
  logoHolder.BackgroundColor3 = T0[5]
  logoHolder.ZIndex = 502
  logoHolder.Parent = card
  Instance.new("UICorner", logoHolder).CornerRadius = UDim.new(0,12)
  local logoBars = Instance.new("Frame")
  logoBars.Size = UDim2.new(0,24,0,24)
  logoBars.AnchorPoint = Vector2.new(0.5,0.5)
  logoBars.Position = UDim2.new(0.5,0,0.5,0)
  logoBars.BackgroundTransparency = 1
  logoBars.ZIndex = 503
  logoBars.Parent = logoHolder
  local logoLay = Instance.new("UIListLayout", logoBars)
  logoLay.FillDirection = Enum.FillDirection.Horizontal
  logoLay.VerticalAlignment = Enum.VerticalAlignment.Center
  logoLay.HorizontalAlignment = Enum.HorizontalAlignment.Center
  logoLay.Padding = UDim.new(0,3)
  for _, h in ipairs({10,20,14}) do
    local bar = Instance.new("Frame")
    bar.Size = UDim2.new(0,4,0,h)
    bar.BackgroundColor3 = Color3.new(1,1,1)
    bar.BorderSizePixel = 0
  ...
  local ttl = Instance.new("TextLabel")
  ttl.Text = "Choose interface"
  ttl.Size = UDim2.new(1,-20,0,20)
  ttl.Position = UDim2.new(0,10,0,72)
  ...
  local function pickBtn(label, sub, y, mode)
    local b = Instance.new("TextButton")
    b.Size = UDim2.new(1,-32,0,44)
    b.Position = UDim2.new(0,16,0,y)
    b.BackgroundColor3 = T0[4]
    b.Text = ""
    ...
    l2.Position = UDim2.new(0,10,0,22)
  ...
  pickBtn("1. Classic", "Sidebar layout, full features", 100, "classic")
  pickBtn("2. New", "Compact touch layout", 150, "new")
end

local function showLoadingScreen(onDone)
  local lgui = Instance.new("ScreenGui")
  lgui.Name = "RiseLoadGate"
  lgui.ResetOnSpawn = false
  lgui.DisplayOrder = 998
  lgui.Parent = PG
  ...
  logoHolder.Position = UDim2.new(0.5,0,0.4,0)
  logoHolder.BackgroundColor3 = Color3.fromRGB(180,70,255)
  logoHolder.ZIndex = 401
  logoHolder.Parent = bg
  Instance.new("UICorner", logoHolder).CornerRadius = UDim.new(0,16)
  local logoBars = Instance.new("Frame")
  logoBars.Size = UDim2.new(0,32,0,32)
  logoBars.AnchorPoint = Vector2.new(0.5,0.5)
  logoBars.Position = UDim2.new(0.5,0,0.5,0)
  logoBars.BackgroundTransparency = 1
  logoBars.ZIndex = 402
  ...
  logoLay.VerticalAlignment = Enum.VerticalAlignment.Center
  logoLay.HorizontalAlignment = Enum.HorizontalAlignment.Center
  logoLay.Padding = UDim.new(0,4)
  for _, h in ipairs({14,28,20}) do
    local bar = Instance.new("Frame")
    bar.Size = UDim2.new(0,6,0,h)
    bar.BackgroundColor3 = Color3.new(1,1,1)
    bar.BorderSizePixel = 0
    bar.ZIndex = 402
    bar.Parent = logoBars
  ...
  title.Text = "RISE ULTIMATE"
  title.Size = UDim2.new(0,300,0,30)
  title.AnchorPoint = Vector2.new(0.5,0.5)
  title.Position = UDim2.new(0.5,0,0.51,0)
  ...
  spinnerHolder.Size = UDim2.new(0,60,0,60)
  spinnerHolder.AnchorPoint = Vector2.new(0.5,0.5)
  spinnerHolder.Position = UDim2.new(0.5,0,0.65,0)
  spinnerHolder.BackgroundTransparency = 1
  spinnerHolder.ZIndex = 401
  spinnerHolder.Parent = bg
  ...
  task.spawn(function()
    local idx = 1
    while spinning do
      for i=1,N do spinBars[i].BackgroundTransparency = 0.85 end
      spinBars[idx].BackgroundTransparency = 0
      spinBars[(idx-2)%N+1].BackgroundTransparency = 0.4
      idx = idx % N + 1
      task.wait(0.09)
    end
  end)
  ...
  skipBtn.AnchorPoint = Vector2.new(0.5,1)
  skipBtn.Position = UDim2.new(0.5,0,1,-30)
  ...
  local elapsed = 0
  while elapsed < 5 and not skipped do
    task.wait(0.1)
    elapsed = elapsed + 0.1
  end
  spinning = false
